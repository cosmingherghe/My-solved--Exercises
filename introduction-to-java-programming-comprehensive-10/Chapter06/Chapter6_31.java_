...
public class CreditCardNumberValidation {
	...
	public static void main(String[] args) {
		...
		System.out.println( isValid(4388576018402626) ); //4388576018402626 is invalid
		System.out.println( isValid(4388576018410707) ); //4388576018410707 is valid
	}
	...
	public static int sumOfDoubleEvenPlace(long number) {
		...
		do{
			if(number > 100) { 
				sumOfDoubleEvenPlace += getDigit( (int) ((number % 100) / 10) * 2 );
				number /= 100;
			}
			else{
				sumOfDoubleEvenPlace += getDigit( (int) (number / 10) * 2 );
				number /= 10;
			}  
		}while(number > 10);
	...
	* return the sum of the two digits */
	...
	/** Return sum of oddd-place igits in number */
	public static int sumOfOddPlace(long number) {
		int sumOfOddPlace = 0; 
	...
	public static boolean prefixMatched(long number, int d) {
		...
		for(int i = 0; i < digits.length(); i++) {
			if(cardNumber.charAt(i) != digits.charAt(i))
				return false;
		}
		return true;
	}
	...
	* number of digits in number is less than k, return number. */
	public static long getPrefix(long number, int k) {
		...
		if(cardNumber.length() >= k) {
			number = cardNumber.charAt(0);
			
			for(int i=1; i<k;i++){
				number +=  (long)cardNumber.charAt(i) * 10; 
			}
